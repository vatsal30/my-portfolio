...
export async function getNotesList(): Promise<NoteMeta[]> {
    try {
        ...
        const rootRes = await fetch(`https://api.github.com/repos/${NOTES_REPO}/contents`, { headers, next: { revalidate: 3600 } });
        if (!rootRes.ok) {
        ...
        const directories = rootItems.filter((item: any) => item.type === "dir" && !item.name.startsWith("."));

        let allNotes: NoteMeta[] = [];

        // 3. Fetch contents of each directory
        for (const dir of directories) {
            const dirRes = await fetch(`https://api.github.com/repos/${NOTES_REPO}/contents/${dir.path}`, { headers, next: { revalidate: 3600 } });
            if (!dirRes.ok) continue;

            const dirItems = await dirRes.json();
            const mdFiles = dirItems.filter((item: any) => item.type === "file" && item.name.endsWith(".md"));
...
export async function getNoteContent(slug: string): Promise<string | null> {
    try {
        ...
        const res = await fetch(`https://api.github.com/repos/${NOTES_REPO}/contents/${note.path}`, { headers, next: { revalidate: 3600 } });
